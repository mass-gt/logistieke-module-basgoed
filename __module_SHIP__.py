...
def actually_run_module(args):
    ...
    try:
        ...
        basGoedParamPath = varDict['BASGOED_PARAMETERS']
        basGoedCKMpath   = varDict['BASGOED_CKM']
        basGoedMSpath    = varDict['BASGOED_MS']
        ...
        terminalsPath         = varDict['TERMINALS']
        vehCapacityPath    = varDict['VEHCAPACITY']
        convSectorPath     = varDict['SECTOR_TO_SECTOR']
        containerStatsPath = varDict['CONTAINER']
        flowTypesPath  = varDict['FLOWTYPES']
        paramsSSVTpath = varDict['PARAMS_SSVT']
        yearFac  = varDict['YEARFACTOR']
        year     = varDict['YEAR']
        baseYear = varDict['BASEYEAR']

        NRMtoBGpath     = varDict['NRM_TO_BG']
        BGtoNRMpath     = varDict['BG_TO_NRM']
        GGtoNSTRcontPath  = varDict['BGGG_TO_NSTR_CONT']
        GGtoNSTRncontPath = varDict['BGGG_TO_NSTR_NCONT']
        GGtoLScontPath  = varDict['BGGG_TO_LS_CONT']
        GGtoLSncontPath = varDict['BGGG_TO_LS_NCONT']

        costAanhangerPath   = varDict['COST_AANHANGER']
        costBestelPath      = varDict['COST_BESTEL']
        costLZVpath         = varDict['COST_LZV']
        costOpleggerPath    = varDict['COST_OPLEGGER']
        costSpeciaalPath    = varDict['COST_SPECIAAL']
        costVrachtwagenPath = varDict['COST_VRACHTWAGEN']
        ...
        forecast = (year > baseYear)
            
        jobNames = ['LANDBOUW'  + yrAppendix,
        ...
        convGG = np.array(pd.read_csv(basGoedParamPath + "Goederengroep.asc", sep='\t'), dtype=int)
        convGG = dict((convGG[i,:]) for i in range(len(convGG)))
        convSector = np.array(pd.read_csv(convSectorPath, sep=','))
        convSector[:,1] = [jobDict[x + yrAppendix] for x in convSector[:,1]]
        convSector = dict((int(convSector[i,0]),int(convSector[i,1])) for i in range(len(convSector)))
        
        # Import make-use table BasGoed
        makeUseTable = np.array(pd.read_csv(basGoedParamPath + "MakeUseEerstJr.asc", sep='\t'))
        ...
        makeUseTable = makeUseTable[(makeUseTable[:,0]>=0) & (makeUseTable[:,1]>=0), :]
        
        # Create make and use distribution tables (by BG-GG and NRM-industry-sector)
        ...
        # Connectie BasGoed-GG en logistiek segment (kansen, niet cumulatief)
        GGtoLScontProb  = np.array(pd.read_csv(GGtoLScontPath,  sep=',', header=None))
        GGtoLSncontProb = np.array(pd.read_csv(GGtoLSncontPath, sep=',', header=None))
        for gg in range(nGG):            
            if np.sum(GGtoLScontProb[gg,:]) > 0:
                GGtoLScontProb[gg,:] /= np.sum(GGtoLScontProb[gg,:])
            else:
                GGtoLScontProb[gg,:] = (np.ones(nLogSeg) / nLogSeg)
                
            if np.sum(GGtoLSncontProb[gg,:]) > 0:
                GGtoLSncontProb[gg,:] /= np.sum(GGtoLSncontProb[gg,:])
            else:
                GGtoLSncontProb[gg,:] = (np.ones(nLogSeg) / nLogSeg)
        ...
        tonnes = pd.read_csv(basGoedMSpath + f'{filename}1.matrix', sep='\t')[['orig','dest','road']]
        tonnes.columns = ['ORIG','DEST','WeightDay']
        tonnes['GG'] = 1
        for gg in range(2,nGG+1):
            tonnesGG = pd.read_csv(basGoedMSpath + f'{filename}{gg}.matrix', sep='\t')[['orig','dest','road']]
            tonnesGG.columns = ['ORIG','DEST','WeightDay']
            tonnesGG['GG'] = gg
            tonnes = tonnes.append(tonnesGG.copy())
        tonnes['WeightDay'] /= yearFac
        ...
        tonnesLS = np.zeros((nLogSeg*len(tonnes), 5), dtype=float)
        row = 0
        ...
        tonnes = pd.DataFrame(tonnesLS, columns=['ORIG','DEST','GG','LS','WeightDay'])
        tonnes[['ORIG','DEST','GG','LS']] = tonnes[['ORIG','DEST','GG','LS']].astype(int)
        ...
        if forecast:
            ...
            filename = 'Base'
        tonnesCKM = pd.read_csv(basGoedCKMpath + f'{filename}1.matrix', sep='\t')[['orig','dest','road']]
        tonnesCKM.columns = ['ORIG','DEST','WeightDay']
        tonnesCKM['GG'] = 1
        for gg in range(2,nGG+1):
            tonnesGG = pd.read_csv(basGoedCKMpath + f'{filename}{gg}.matrix', sep='\t')[['orig','dest','road']]
            tonnesGG.columns = ['ORIG','DEST','WeightDay']
            tonnesGG['GG'] = gg
            tonnesCKM = tonnesCKM.append(tonnesGG.copy())
        tonnesCKM['WeightDay'] /= yearFac
        ...
        tonnesLS = np.zeros((nLogSeg*len(tonnesCKM), 5), dtype=float)
        row = 0
        ...
        tonnesLS = tonnesLS[tonnesLS[:,4]>0]
        tonnesCKM = pd.DataFrame(tonnesLS, 
                                 columns=['ORIG','DEST','GG','LS','WeightDay'])
        tonnesCKM[['ORIG','DEST','GG','LS']] = tonnesCKM[['ORIG','DEST','GG','LS']].astype(int)

        del tonnesLS, tonnesGG
        
        # Container statistics
        containerStats = pd.read_csv(containerStatsPath, sep=',', index_col='BG_GGR')
        containerSS         = np.array(containerStats['SHIPSIZE_AVG'], dtype=float)[:-1]
        containerEmptyShare = containerStats.iloc[-1,1] / np.sum(containerStats.iloc[:-1,-1])
        ...
        BGtoNRM = pd.read_csv(BGtoNRMpath, sep=',')
        BGtoNRM = BGtoNRM.astype(int)
        BGtoNRM.index = BGtoNRM['ZONEBG2018']
        dictBGtoNRM = {}
        for i in BGtoNRM.index:
            if i in BGwithOverlapNRM:
                dictBGtoNRM[i] = np.array(NRMtoBG.loc[NRMtoBG['ZONEBG2018']==i, 'SEGNR_2018'])
            else:
                dictBGtoNRM[i] = np.array([BGtoNRM.at[i,'SEGNR_2018']])
        
        # Connectie BasGoed-GG en NSTR (cumulatieve kansen)
        GGtoNSTRcont =  np.array(pd.read_csv(GGtoNSTRcontPath,  sep=',', header=None))
        GGtoNSTRncont = np.array(pd.read_csv(GGtoNSTRncontPath, sep=',', header=None))
        for gg in range(nGG):
        ...
        for i in zones.index:
            urbanDensity = 100 * (zones.at[i,'INWONERS' + yrAppendix] / zones.at[i,'OPP'])
            if urbanDensity < 500:
                urbanDensityCat[i] = 1
        ...
        logNodes['ZONEBG2018'] = [dictNRMtoBG[x] for x in logNodes['SEGNR_2018']]
        ...
        terminals = pd.read_csv(terminalsPath, sep=',')
        terminals['ZONEBG2018'] = [dictNRMtoBG[x] for x in terminals['NRM_Moederzone']]
        ...
        industrieFields = ['INDUSTRIE', 'INDUSTRIE_1', 'INDUSTRIE_2', 'INDUSTRIE_3']
        ...
        ftSharesNL  = np.array(pd.read_csv(flowTypesPath, index_col=0))[:nFlowTypesInternal,:]
        ftSharesExt = np.array(pd.read_csv(flowTypesPath, index_col=0))[nFlowTypesInternal:,:]
        
        # Account flowtypes for BG-zones without DC or TT (within NL)
        tonnesNL = tonnes[(tonnes['ORIG'] <= nZonesNL_BG) & (tonnes['DEST'] <= nZonesNL_BG)].copy()
        for ls in range(nLogSeg):
            tonnesNL_LS = tonnesNL[tonnesNL['LS']==ls]
            sumTonnesNL = np.sum(tonnesNL_LS['WeightDay'])
            
            whereFromDC = [i for i in tonnesNL_LS.index if tonnesNL_LS.at[i,'ORIG'] in BGzonesWithLogNode]
            sumTonnesNLfromDC = np.sum(tonnesNL_LS.loc[whereFromDC,'WeightDay'])
            fromDCfac = sumTonnesNL / sumTonnesNLfromDC
            ftSharesNL[[3-1, 5-1, 7-1], ls] *= fromDCfac

            whereToDC = [i for i in tonnesNL_LS.index if tonnesNL_LS.at[i,'DEST'] in BGzonesWithLogNode]
            sumTonnesNLtoDC = np.sum(tonnesNL_LS.loc[whereToDC, 'WeightDay'])
            toDCfac = sumTonnesNL / sumTonnesNLtoDC
            ftSharesNL[[2-1, 5-1, 8-1], ls] *= toDCfac
            
            whereFromTT = [i for i in tonnesNL_LS.index if tonnesNL_LS.at[i,'ORIG'] in BGzonesWithTerminal]
            sumTonnesNLfromTT = np.sum(tonnesNL_LS.loc[whereFromTT, 'WeightDay'])
            fromTTfac = sumTonnesNL / sumTonnesNLfromTT
            ftSharesNL[[6-1, 8-1, 9-1], ls] *= fromTTfac

            whereToTT = [i for i in tonnesNL_LS.index if tonnesNL_LS.at[i,'DEST'] in BGzonesWithTerminal]
            sumTonnesNLtoTT = np.sum(tonnesNL_LS.loc[whereToTT, 'WeightDay'])
            toTTfac = sumTonnesNL / sumTonnesNLtoTT
            ftSharesNL[[4-1, 7-1, 9-1], ls] *= toTTfac
        
        # Account flowtypes for BG-zones without DC or TT (from/to NL)
        tonnesExt = tonnes[ ((tonnes['ORIG']>nZonesNL_BG) | (tonnes['DEST']>nZonesNL_BG)) & 
                           ~((tonnes['ORIG']>nZonesNL_BG) & (tonnes['DEST']>nZonesNL_BG))].copy()
        for ls in range(nLogSeg):
            tonnesExtLS = tonnesExt[tonnesExt['LS']==ls]
            sumTonnesExtToNL   = np.sum(tonnesExtLS.loc[tonnesExtLS['DEST']<=nZonesNL_BG, 'WeightDay'])
            sumTonnesExtFromNL = np.sum(tonnesExtLS.loc[tonnesExtLS['ORIG']<=nZonesNL_BG, 'WeightDay'])
            
            whereExtFromDC = [i for i in tonnesExtLS.index if tonnesExtLS.at[i,'ORIG'] in BGzonesWithLogNode]
            whereExtToDC   = [i for i in tonnesExtLS.index if tonnesExtLS.at[i,'DEST'] in BGzonesWithLogNode]
            sumTonnesExtFromDC = np.sum(tonnesExtLS.loc[whereExtFromDC, 'WeightDay'])
            sumTonnesExtToDC   = np.sum(tonnesExtLS.loc[whereExtToDC,   'WeightDay'])
            ...
            whereExtFromTT = [i for i in tonnesExtLS.index if tonnesExtLS.at[i,'ORIG'] in BGzonesWithTerminal]
            whereExtToTT   = [i for i in tonnesExtLS.index if tonnesExtLS.at[i,'DEST'] in BGzonesWithTerminal]
            sumTonnesExtFromTT = np.sum(tonnesExtLS.loc[whereExtFromTT, 'WeightDay'])            
            sumTonnesExtToTT   = np.sum(tonnesExtLS.loc[whereExtToTT,   'WeightDay'])
        ...
        costFreight[0] = np.array(pd.read_csv(costVrachtwagenPath, sep='\t'))[:,:3]
        costFreight[1] = costFreight[0]
        costFreight[2] = costFreight[0]
        costFreight[3] = np.array(pd.read_csv(costAanhangerPath, sep='\t'))[:,:3]
        costFreight[4] = costFreight[3]
        costFreight[5] = np.array(pd.read_csv(costOpleggerPath, sep='\t'))[:,:3]
        costFreight[6] = np.array(pd.read_csv(costSpeciaalPath, sep='\t'))[:,:3]
        costFreight[7] = np.array(pd.read_csv(costLZVpath, sep='\t'))[:,:3]
        costFreight[8] = np.array(pd.read_csv(costBestelPath, sep='\t'))[:,:3]
        ...
        paramsShipSizeVehType = pd.read_csv(paramsSSVTpath, index_col=0)
        ...
        truckCapacities = np.array(pd.read_csv(vehCapacityPath, index_col=0))[:,0] / 1000
        absoluteShipmentSizes = np.array([1.5, 4.5, 8.0, 15.0, 25.0, 35.0], dtype=float)
        ...
        # ------------- Shipment synthesizer procedure (non-containerized) ------------------
        
        if shipmentsRef == "":
            
            print("Calculating zonal probabilities..."), log_file.write("Calculating zonal probabilities...\n")               
            if root != '':
                root.update_statusbar("Shipment Synthesizer: Calculating zonal probabilities")

            # Determine probabilities of zones/DCs/TTs being origin of a shipment            
            origProbs = {}
            origProbsDC = {}
            origProbsTT = {}
            
            for origZoneBG in np.unique(tonnes['ORIG'].append(tonnesCKM['ORIG'])):
                ...
                for gg in range(nGG):
                    origProbs[origZoneBG][gg]  = np.sum(makeDistribution[gg,:] * origJobsNRM, axis=1)
                    if np.sum(origProbs[origZoneBG][gg]) == 0:
                        origProbs[origZoneBG][gg] = np.ones(len(origProbs[origZoneBG][gg]))
                    origProbs[origZoneBG][gg]  = np.cumsum(origProbs[origZoneBG][gg])
                ...
                if origZoneBG in BGzonesWithLogNode:
                    ...
                    origProbsDC[origZoneBG]   = np.cumsum(origProbsDC[origZoneBG])
                ...
                if origZoneBG in BGzonesWithTerminal:
                    ...
                    origProbsTT[origZoneBG]   = np.cumsum(origProbsTT[origZoneBG])
                    origProbsTT[origZoneBG]  /= origProbsTT[origZoneBG][-1]

            if root != '':
                root.progressBar['value'] = 4
                
            # Determine probabilities of zones/DCs/TTs being destination of a shipment            
            destProbs = {}
            destProbsDC = {}
            destProbsTT = {}
            
            for destZoneBG in np.unique(tonnes['DEST'].append(tonnesCKM['DEST'])):  
                # Which NRM-zones fall under the current BG-zones
                ...
                for gg in range(nGG):
                    destProbs[destZoneBG][gg]  = np.sum(useDistribution[gg,:] * destJobsNRM, axis=1)
                    if np.sum(destProbs[destZoneBG][gg]) == 0:
                        destProbs[destZoneBG][gg] = np.ones(len(destProbs[destZoneBG][gg]))
                    destProbs[destZoneBG][gg]  = np.cumsum(destProbs[destZoneBG][gg])
                ...
                if destZoneBG in BGzonesWithLogNode:
                    ...
                    destProbsDC[destZoneBG]   = np.cumsum(destProbsDC[destZoneBG])
                ...
                if destZoneBG in BGzonesWithTerminal:
                    ...
                    destProbsTT[destZoneBG]   = np.cumsum(destProbsTT[destZoneBG])
            ...
            print("Synthesizing shipments MS..."), log_file.write("Synthesizing shipments MS...\n")               
            percStart = 5
            percEnd   = 80
            if root != '':
                root.progressBar['value'] = percStart
                root.update_statusbar("Shipment Synthesizer: Synthesizing shipments MS (0.0%)")
            ...
            for i in tonnes.index:
                ...
                # Keep track of allocated weight for current cell (i.e. orig-dest-goods type combinations)
                ...
                if recalculateDestZones:
                    # Which NRM-zones fall under the current BG-destination-zone
                    destZonesNRM = dictBGtoNRM[destZoneBG]                    
                ...
                while allocatedWeightCurrentCell < totalWeightCurrentCell:
                    ...
                    if origZoneBG > nZonesNL_BG or destZoneBG > nZonesNL_BG:
                        if origZoneBG > nZonesNL_BG and destZoneBG > nZonesNL_BG:
                            ft = 10
                            fromExt = 1
                            toExt   = 1
                    ...
                    if ft in (3, 5, 7):
                        if DCsInOrig:
                            origDC      = whereDCinOrig[draw_choice(origProbsDC[origZoneBG])]
                    ...
                    if ft in (6, 8, 9):
                        if TTsInOrig:
                            origTT      = whereTTinOrig[draw_choice(origProbsTT[origZoneBG])]
                    ...
                    if ft in (1, 2, 4):                 
                        origZoneNRM = origZonesNRM[draw_choice(origProbs[origZoneBG][goodsTypeBG-1])]
                        fromDC = 0
                        fromTT = 0                        
                    ...
                    if ft in (2, 5, 8):
                        if DCsInDest:
                            destDC      = whereDCinDest[draw_choice(destProbsDC[destZoneBG])]
                    ...
                    if ft in (4, 7, 9):
                        if TTsInDest:
                            destTT      = whereTTinDest[draw_choice(destProbsTT[destZoneBG])]
                    ...
                    if ft in (1, 3, 6):                      
                        destZoneNRM = destZonesNRM[draw_choice(destProbs[destZoneBG][goodsTypeBG-1])]                    
                        toDC = 0
                        toTT = 0
                    ...
                    # TO DO: Select parcel nodes for logistic segment 6
                    
                    
                    
                    # Replacement value for origDC/TT and destDC/TT if the flow doesn't
                    # go or come from a DC/TT
                    if fromDC == 0:
                        origDC = -9999
                    if fromTT == 0:
                        origTT = -9999
                    ...
                    # Selecting the logit parameters for this BasGoed goods type
                    paramsShipSizeVehTypeGG   = paramsShipSizeVehType.iloc[:, goodsTypeBG-1]
                    B_TransportCosts          = paramsShipSizeVehTypeGG['B_TransportCosts']
                    B_InventoryCosts          = paramsShipSizeVehTypeGG['B_InventoryCosts']
                    B_FromDC                  = paramsShipSizeVehTypeGG['B_FromDC']
                    B_ToDC                    = paramsShipSizeVehTypeGG['B_ToDC']
                    B_LongHaul_TruckTrailer   = paramsShipSizeVehTypeGG['B_LongHaul_TruckTrailer']
                    B_LongHaul_TractorTrailer = paramsShipSizeVehTypeGG['B_LongHaul_TractorTrailer']
                    ASC_VT = np.array([paramsShipSizeVehTypeGG[f'ASC_VT_{i+1}'] for i in range(nVT)], dtype=float)
                    
                    # Determine values for attributes in the utility function of the shipment size/vehicle type MNL
                    travTime = skimTravTime[(origZoneNRM-1)*nZones + (destZoneNRM-1)] / 3600
                    distance = skimDistance[(origZoneNRM-1)*nZones + (destZoneNRM-1)] / 1000
                    
                    # Draw one choice for shipment size and vehicle type
                    ssvt = choice_model_ssvt(costFreightKm, costFreightHr, 
                                             goodsTypeBG, fromDC, toDC,
                                             travTime, distance,
                                             absoluteShipmentSizes, truckCapacities,
                                             B_TransportCosts, B_InventoryCosts, B_FromDC, B_ToDC,
                    ...
                    shipmentSize    = min(absoluteShipmentSizes[ssChosen], 
                    ...
                    if count%500 == 0:
                        ...
                        if root != '':                        
                            root.progressBar['value'] = percStart + (percEnd - percStart) * (allocatedWeight / totalWeight)
                            root.update_statusbar(f"Shipment Synthesizer: Synthesizing shipments MS ({progress}%)")
 
            nShipsMS = count
            
            
            # --------------- Shipment synthesizer procedure (containerized) --------------------
            
            print("Synthesizing shipments CKM..."), log_file.write("Synthesizing shipments CKM...\n")               
            percStart = 80
            percEnd   = 90
            if root != '':
                root.progressBar['value'] = percStart
                root.update_statusbar("Shipment Synthesizer: Synthesizing shipments CKM (0.0%)")
            ...
            for i in tonnesCKM.index:
                ...
                # Total weight for current cell (i.e. orig-dest-goods type combination)
                totalWeightCurrentCell = tonnesCKM.at[i,'WeightDay']
                
                # Split weight by empty and loaded containers
                totalWeightCurrentCellLoaded = (1 - containerEmptyShare) * totalWeightCurrentCell
                totalWeightCurrentCellEmpty  = containerEmptyShare       * totalWeightCurrentCell
                
                # Determine number of shipments
                nShipsCurrentCellLoaded = max(1, int(np.round(totalWeightCurrentCellLoaded / containerSS[goodsTypeBG-1], 0)))
                nShipsCurrentCellEmpty  =        int(np.round(totalWeightCurrentCellEmpty  / containerEmptySS          , 0))
                nShipsCurrentCell       = nShipsCurrentCellLoaded + nShipsCurrentCellEmpty
                
                if nShipsCurrentCellLoaded > 0:
                    shipSizeCurrentCellLoaded = totalWeightCurrentCellLoaded / nShipsCurrentCellLoaded
                else:
                    shipSizeCurrentCellLoaded = containerSS[goodsTypeBG-1]
                
                if nShipsCurrentCellEmpty > 0:
                    shipSizeCurrentCellEmpty = totalWeightCurrentCellEmpty  / nShipsCurrentCellEmpty
                else:
                ...
                for ship in range(nShipsCurrentCell):
                    ...
                    # The chosen vehicle type (always tractor+trailer for containerized shipments)
                    vehicleType = 5
                    ...
                    if count%500 == 0:
                        ...
                        if root != '':                        
                            root.progressBar['value'] = percStart + (percEnd - percStart) * (allocatedWeight / totalWeight)
                            root.update_statusbar(f"Shipment Synthesizer: Synthesizing shipments CKM ({progress}%)")
            ...
            if root != '':                        
                ...
                root.update_statusbar("Shipment Synthesizer: Synthesizing shipments CKM (100.0%)")
            ...
            temp, shipCoords = read_shape(shipmentsRef[:-4] + '.shp', returnGeometry=True)
            shipCoords = [shipCoords[i]['coordinates'] for i in range(len(shipCoords))]
            ship_origX = dict((i, shipCoords[i][0][0]) for i in range(len(shipCoords)))
            ship_origY = dict((i, shipCoords[i][0][1]) for i in range(len(shipCoords)))
            ship_destX = dict((i, shipCoords[i][1][0]) for i in range(len(shipCoords)))
            ship_destY = dict((i, shipCoords[i][1][1]) for i in range(len(shipCoords)))
        ...
        if label == 'UCC':
            ...
            whereOrigZEZ = [i for i in shipments[shipments['ORIG_BG']<=nZonesNL_BG].index
                            if shipments.at[i, 'ORIG_NRM'] in zezZones]
            whereOrigZEZ = np.array(whereOrigZEZ, dtype=int)
            whereDestZEZ = [i for i in shipments[shipments['DEST_BG']<=nZonesNL_BG].index
            ...
            for i in whereBothZEZ:
                ...
                if zones['GEM2019'][shipments['ORIG_NRM'][i]] == zones['GEM2019'][shipments['DEST_NRM'][i]]:                    
                    if ls != 7:
            ...
            newShipments['ORIG_LMS'] = [zonesLMS[int(x - 1)] for x in newShipments['ORIG_NRM'].values]
            newShipments['DEST_LMS'] = [zonesLMS[int(x - 1)] for x in newShipments['DEST_NRM'].values]
