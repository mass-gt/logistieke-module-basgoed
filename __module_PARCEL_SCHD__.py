...
def actually_run_module(args):
    ...
    try:
        ...
        depTimeParcelsPath = varDict['DEPTIME_PARCELS']
        label            = varDict['LABEL']
        ...
        print('\tParcel demand...')
        parcelsAgg = pd.read_csv(datapathO + 'ParcelDemand_' + label + '.csv')
        nParcelsTotal = np.sum(parcelsAgg['N_PARCELS'])
        
        # Parcels herstructureren zodat iedere rij 1 parcel is
        parcelColumns = ['PARCEL_ID', 'ORIG_NRM', 'DEST_NRM', 'DEPOT_ID', 'CEP', 'VEHTYPE']
        if label == 'UCC':
        ...
        parcels = parcels.astype({'PARCEL_ID': int, 
                                  'ORIG_NRM': int, 
                                  'DEST_NRM': int, 
                                  'DEPOT_ID': int, 
                                  'CEP':str,
        ...
        parcelDepTime = np.array(pd.read_csv(depTimeParcelsPath).iloc[:,1])        
        ...
        skimEuclidean  = ((zonesX.repeat(nZones).reshape(nZones, nZones) -
                           zonesX.repeat(nZones).reshape(nZones, nZones).transpose()) / 1000)**2
        skimEuclidean += ((zonesY.repeat(nZones).reshape(nZones, nZones) -
                           zonesY.repeat(nZones).reshape(nZones, nZones).transpose()) / 1000)**2
        ...
        if label == 'UCC':
            ...
            parcelsUCC[2] = pd.DataFrame(parcels[(parcels['FROM_UCC']==1) & (parcels['VEHTYPE']==8)])
            parcelsUCC[3] = pd.DataFrame(parcels[(parcels['FROM_UCC']==1) & (parcels['VEHTYPE']>=9)])
            ...
            maxVehicleLoadLEVV = int(round(maxVehicleLoad / 5))
            parcelsUCC[3] = cluster_parcels(parcelsUCC[3], maxVehicleLoadLEVV, skimClustering,
                                            root, startValueProgress, endValueProgress)
        ...
        deliveries['TOUR_DEPTIME'] = [round(deliveries['TOUR_DEPTIME'][i], 3) for i in deliveries.index]
        deliveries['TRIP_DEPTIME'] = [round(deliveries['TRIP_DEPTIME'][i], 3) for i in deliveries.index]
        ...
        pivotTable = pd.pivot_table(deliveries, values=['N_TOT'], index=['ORIG_NRM','DEST_NRM'], aggfunc=np.sum)
        ...
        pivotTable.to_csv(datapathO + f"TripMatrix_ParcelVans_NRM_{label}.csv", index=False, sep=',')
        
        print(f'Trip matrix written to {datapathO}TripMatrix_ParcelVans_NRM_{label}.csv')
        log_file.write(f'Trip matrix written to {datapathO}TripMatrix_ParcelVans_NRM_{label}.csv\n')
...
def create_schedules(parcelsAgg, dropOffTime, skimTravTime, skimDistance, parcelNodesCEP, parcelDepTime,
    ...
    for depot in np.unique(parcelsAgg['Depot']):
        ...
        for cluster in np.unique(depotParcels['Cluster']):
            ...
            for i in range(len(destZones)):
                distances = [skimDistance[(tour[i]-1) * nZones + (dest-1)] for dest in destZones]
            ...
            tourDist = np.sum(skimDistance[(tour[:-1]-1) * nZones + (tour[1:]-1)])
            if nStops > 4:
            ...
            for i in range(1, nStops-1):
                orig = tour[i-1]
                dest = tour[i]
                travTime = skimTravTime[(orig-1) * nZones + (dest-1)] / 3600
        ...
        print('\t' + str(round((depotCount+1)/nDepots*100, 1)) + '%', end='\r')
    ...
    # ------------------------------ Create return table ---------------------- 
    deliveriesCols = ['TOUR_TYPE',  'CEP',          'DEPOT_ID',     'TOUR_ID', 
    ...
    for depot in tours.keys():        
        for tour in tours[depot].keys():
            for trip in range(len(tours[depot][tour])-1):
                ...
                deliveries[tripcount, 9] = skimTravTime[(orig-1) * nZones + 
                                                        (dest-1)] / 3600             # Travel time in hrs
    ...
    dtypes =  {'TOUR_TYPE':int,   'CEP':str,        'DEPOT_ID':int,       'TOUR_ID':str, 
               'TRIP_ID':str,     'UNIQUE_ID':str,  'ORIG_NRM':int,       'DEST_NRM':int,       
               'N_PARCELS':float, 'TRAVTIME':float, 'TOUR_DEPTIME':float, 'TRIP_DEPTIME':float}
...
def cluster_parcels(parcels, maxVehicleLoad, skimDistance,
    ...
    counts = pd.pivot_table(parcels, values=['VEHTYPE'], index=['DEPOT_ID','DEST_NRM'], aggfunc=len)
    ...
    for x in whereLargeCluster:
        ...
        for i in range(int(np.floor(len(indices)/maxVehicleLoad))):
            ...
            print('\t' + str(round((nParcelsAssigned/nParcels)*100, 1)) + '%', end='\r')        
            if root != '':
                root.progressBar['value'] = startValueProgress + \
                                            (endValueProgress - startValueProgress - 1) * \
    ...
    for depotNumber in depotNumbers:
        ...
        parcelsToFit = parcels[(parcels['DEPOT_ID']==depotNumber) & 
        ...
        parcelsToFit['Distance'] = skimDistance[(parcelsToFit['ORIG_NRM']-1) * nZones + 
