...
# Module ongebruikt maar nodig voor bij het compileren
import psutil
...
class Root:
    ...
    def __init__(self):
        ...
        self.root = tk.Tk()
        self.root.title(
            "Logistieke Module BasGoed (Prototype 9 december 2021)")
    ...
    def actually_run_main(self):
        ...
        '''
        # Welke parameters zijn numeriek of stellen een directory/filenaam voor
        numericVars = [
            ...
            "YEAR", "BASEYEAR"]
        dirVars = [
            "OUTPUTFOLDER", "PARAMFOLDER", "DIMFOLDER",
            "BASGOED_PARAMETERS", "BASGOED_CKM", "BASGOED_MS"]
        moduleVars = [
            "MODULES"]
        fileVars = [
            ...
            "LINKS", "NODES",
            "PARCELNODES",
            "DISTRIBUTIECENTRA", "TERMINALS",
            "SERVICE_PA", "SERVICE_DISTANCEDECAY",
            "SECTOR_TO_SECTOR",
            "CONTAINER",
            "CEP_SHARES",
            "FLOWTYPES",
            "VEHCAPACITY",
            "NRM_TO_BG", "BG_TO_NRM",
            "BGGG_TO_NSTR_CONT", "BGGG_TO_NSTR_NCONT",
            "BGGG_TO_LS_CONT", "BGGG_TO_LS_NCONT",
            "COST_ROAD_MS", "COST_ROAD_CKM",
            "COST_AANHANGER", "COST_BESTEL", "COST_LZV",
            "COST_OPLEGGER", "COST_SPECIAAL", "COST_VRACHTWAGEN",
            "DEPTIME_PARCELS", "DEPTIME_FREIGHT",
            "PARAMS_ET_FIRST", "PARAMS_ET_LATER", "PARAMS_SSVT",
            "EM_BUITENWEG_LEEG", "EM_BUITENWEG_VOL",
            "EM_SNELWEG_LEEG", "EM_SNELWEG_VOL",
            "EM_STAD_LEEG", "EM_STAD_VOL",
            "ZEZ_CONSOLIDATION", "ZEZ_SCENARIO", "ZEZ_ZONES",
            "SHIPMENTS_REF"]
        optionalVars = [
            "SHIPMENTS_REF",
            "ZEZ_CONSOLIDATION", "ZEZ_SCENARIO", "ZEZ_ZONES",
        ...
        # Lijst met alle mogelijke sleutels in de control file
        self.varStrings = np.unique(
            numericVars + dirVars + moduleVars +
            fileVars + optionalVars + otherVars)
        nVars = len(self.varStrings)

        # Op welke index staat de OUTPUTFOLDER sleutel
        whereOutputFolder = [
            i for i in range(nVars)
            if self.varStrings[i] == "OUTPUTFOLDER"][0]
        whereParamFolder = [
            i for i in range(nVars)
            if self.varStrings[i] == "PARAMFOLDER"][0]
        whereDimFolder = [
            i for i in range(nVars)
            if self.varStrings[i] == "DIMFOLDER"][0]

        # Hier stoppen we de waardes behorende bij de sleutels
        varValues = ["" for i in range(nVars)]

        acceptedRunYears = [2018, 2030, 2040, 2050]

        # Wel of niet runnen, wordt op False gezet als
        # bijv. bestanden niet gevonden kunnen worden
        run = True

        # Wel of geen logfile schrijven, wordt op False gezet als
        # de outputfolder niet bestaat
        writeLog = True

        # Hierin verzamelen we alle foutmeldingen in
        errorMessage = ""
        ...
        try:
                        ...
                        if line[0] != '#':
                            ...
                            for i in range(nVars):
                                if key.upper() == self.varStrings[i]:
                                    ...
                                    # The argument which states which modules
                                    # should be run
                                    ...
                                    # possible '\' by '/'
                                        ...
                                        if (
                                            self.varStrings[i] in dirVars or
                                            self.varStrings[i] in fileVars
                                        ):
                                            if self.varStrings[i] not in ['OUTPUTFOLDER', 'PARAMFOLDER', 'DIMFOLDER']:
            ...
            for i in range(nVars):
                ...
                if self.varStrings[i] in dirVars:
                    if not os.path.isdir(varValues[i]) and varValues[i] != "":
                        ...
                        # Als de opgegeven outputfolder niet bestaat
                        # dan kunnen we er ook geen logfile in schrijven
                ...
                if self.varStrings[i] == 'YEAR':
                    if not varValues[i] in acceptedRunYears:
                        errorMessage = (
                            errorMessage +
                            'Fill in 2018, 2030, 2040 or 2050 ' +
                            'for field YEAR. Got ' +
                            "'" + str(varValues[i]) + "'" +
                            " instead.\n")
                        run = False
            ...
            for i in range(nVars):
                if (
                    varValues[i] == "" and
                    self.varStrings[i] not in optionalVars
                ):
                    errorMessage = (
                        errorMessage +
        ...
        # Open de logfile en schrijf de header, de opgegeven argumenten
        ...
        if writeLog:
            ...
            with open(self.logFileName, "w") as f:
                f.write('########################################################################################\n')
                f.write('### Logistieke Module BasGoed                                                        ###\n')
                f.write('### Prototype 9 December 2021                                                        ###\n')
                f.write('########################################################################################\n')
                f.write('\n')
        ...
        if run:
            ...
            if result[0] == 1:
                self.statusBar.configure(
                    text=(
                        ...
                        "(Tool will be closed automatically after 15 seconds.)"))
                ...
                if (
                    len(varDict['MODULES']) == 1 and
                    varDict['MODULES'][0] == 'INVOERCONTROLE'
                ):
                    self.root.after(15000, lambda: self.root.destroy())
                else:
            ...
            self.statusBar.configure(
                text="Run was not started. See error message. ")
            errorMessage = (
    ...
    def main(self, varDict):
        ...
        with open(self.logFileName, "a") as f:
            ...
            if run and 'KPI' in varDict['MODULES']:
                print('---------------------------------------------------------------------------------')
                ...
                self.statusBar.configure(text='Running KPI module...')

                result = __module_KPI__.actually_run_module(args)
